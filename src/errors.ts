...
const titles: Record<string, string | undefined> = {
    ...
    KotlincPermissionDenied: 'Retussari runner was denied permission to run kotlinc',
...
const messages = {
    KotlincNotFound: 'Check if kotlinc is installed on your machine and reachable with PATH variable.',
    KotlincPermissionDenied: 'Strange. Make sure that file reachable as \'kotlinc\' is valid and executable.',
    UnsupportedPlatform: 'Unsupported platform',
    KillError: 'Kill failed'
}
const complexMessages: ComplexRunErrorMessageProducers = {
    SaveError: (msg: string) => `Saving script to temporary file failed with following message: ${msg}`,
    RemoveError: (msg: string) => `Removing temporary file failed with following message: ${msg}`,
    WaitError: (msg: string) => `Running kotlinc failed with following message: ${msg}`,
    FailedSpawn: (msg: string) => `An error occured while running the script: ${msg}`,
}
...
function describeComplexError<
    ...
    return {
        title: titles[kind],
        message: complexMessages[kind](msg as any),
    }
}
...
function assertNever(err: never): any {
    throw new Error();
}
